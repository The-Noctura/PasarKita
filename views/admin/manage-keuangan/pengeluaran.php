...
$supportsExpense = table_exists('manual_expenses');
...
$editRow = null;
if ($editId > 0 && $supportsExpense) {
    try {
        $stmt = db()->prepare('SELECT id, amount, note, occurred_at FROM manual_expenses WHERE id = :id LIMIT 1');
        ...
        if (is_array($row)) {
            ...
            $errors[] = 'Data pengeluaran tidak ditemukan.';
        }
    } catch (Throwable $e) {
        $errors[] = $e->getMessage();
    }
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$supportsExpense) {
            throw new RuntimeException('Tabel manual_expenses belum tersedia. Jalankan migration terlebih dahulu.');
        }

        $action = (string) ($_POST['action'] ?? 'add');

    $returnUrl = build_return_url('/admin/manage-keuangan/pengeluaran', trim((string) ($_POST['date_range'] ?? '')));
        ...
        if ($action === 'delete') {
            ...
            $stmt = db()->prepare('DELETE FROM manual_expenses WHERE id = :id LIMIT 1');
            $stmt->execute(['id' => $id]);
        ...
        if ($action === 'update') {
            ...
            $stmt = db()->prepare('UPDATE manual_expenses SET amount = :a, note = :n, occurred_at = :d WHERE id = :id LIMIT 1');
            $stmt->execute([
                'a' => $amount,
                'n' => ($note !== '' ? $note : null),
                'd' => $occurredAt,
        ...
        // default: add
        $stmt = db()->prepare('INSERT INTO manual_expenses (amount, note, occurred_at) VALUES (:a, :n, :d)');
        $stmt->execute([
            'a' => $amount,
            'n' => ($note !== '' ? $note : null),
            'd' => $occurredAt,
...
try {
    if ($supportsExpense) {
        ...
        $stmt = db()->prepare('SELECT COALESCE(SUM(COALESCE(amount, 0)), 0) FROM manual_expenses' . $whereSql);
        $stmt->execute($params);
        $totalAmount = (int) ($stmt->fetchColumn() ?? 0);

        $stmt = db()->prepare('SELECT id, amount, note, occurred_at, created_at FROM manual_expenses' . $whereSql . ' ORDER BY occurred_at DESC, id DESC LIMIT 200');
        $stmt->execute($params);
        $rows = $stmt->fetchAll();
...
<body>
    ...
    <div class="main-content">
        <div class="content-header" style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px;">
            ...
            <?php $listUrl = build_return_url('/admin/manage-keuangan/pengeluaran', $filterDateRange); ?>
            <a class="btn-primary" href="<?php echo e(url($listUrl . (str_contains($listUrl, '?') ? '&' : '?') . 'add=1')); ?>">+ Tambah</a>
        </div>

        <?php if (isset($_GET['added'])): ?>
            <div class="message success">Pengeluaran berhasil ditambahkan.</div>
        <?php elseif (isset($_GET['updated'])): ?>
            <div class="message success">Pengeluaran berhasil diupdate.</div>
        <?php elseif (isset($_GET['deleted'])): ?>
            <div class="message success">Pengeluaran berhasil dihapus.</div>
        <?php endif; ?>
        ...
        <div class="recent-transactions">
            <?php if (!$supportsExpense): ?>
                <div class="message">Fitur pengeluaran belum aktif (tabel <b>manual_expenses</b> belum ada di DB yang sedang dipakai aplikasi). Jalankan migration: <code>database/migrations/2026-03-26_add_manual_income_expense.sql</code></div>
            <?php else: ?>
                ...
                <?php if ($showAdd): ?>
                    <div class="form-container" style="margin-bottom:12px;">
                        <form method="POST" class="user-form" action="<?php echo htmlspecialchars(url('/admin/manage-keuangan/pengeluaran')); ?>">
                            <?php echo csrf_field(); ?>
                            ...
                            <?php if (is_array($editRow)): ?>
                                ...
                                <div class="message" style="margin-bottom:12px;">Edit pengeluaran #<?php echo (int) ($editRow['id'] ?? 0); ?>
                                    &nbsp;|&nbsp;
                                    <a href="<?php echo e(url(build_return_url('/admin/manage-keuangan/pengeluaran', $filterDateRange))); ?>">Batal</a>
                                </div>
                            <?php else: ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" min="1" name="amount" id="amount" required placeholder="Contoh: 25000" value="<?php echo e(is_array($editRow) ? (string) ($editRow['amount'] ?? '') : ''); ?>">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="note" id="note" placeholder="Contoh: Bensin / ATK / belanja operasional" value="<?php echo e(is_array($editRow) ? (string) ($editRow['note'] ?? '') : ''); ?>">
                            </div>
                ...
                <?php if (empty($rows)): ?>
                    <div class="message">Belum ada pengeluaran.</div>
                <?php else: ?>
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php foreach ($rows as $r): ?>
                                ...
                                $baseReturn = build_return_url('/admin/manage-keuangan/pengeluaran', $filterDateRange);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" action="<?php echo htmlspecialchars(url('/admin/manage-keuangan/pengeluaran')); ?>" style="margin:0; display:inline;">
                                            <?php echo csrf_field(); ?>
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="id" value="<?php echo (int) $id; ?>">
                                            <input type="hidden" name="date_range" value="<?php echo htmlspecialchars($filterDateRange); ?>">
                                            <button type="submit" class="btn-primary" onclick="return confirm('Hapus pengeluaran ini?');">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
